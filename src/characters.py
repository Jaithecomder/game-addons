...
def move_starchers(V,type):
    if(type == 1):
        for starcher in starchers:
            if(starcher.alive == False):
                continue
            if starcher.target != None:
                if(V.map[starcher.target[0]][starcher.target[1]] == pt.BLANK):
                    starcher.target = None
            if(starcher.target == None):
                starcher.target = search_for_closest_building(starcher.position, V.map, 0)
            if(starcher.target == None):
                continue
            starcher.move(starcher.target, V,type)
    elif(type == 2):
        for starcher in starchers:
            if(starcher.alive == False):
                continue
            closest_building = search_for_closest_building(starcher.position, V.map, 0)
            if(closest_building == None):
                continue
            starcher.move(closest_building, V, type)
